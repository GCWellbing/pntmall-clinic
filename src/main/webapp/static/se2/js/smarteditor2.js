 ...
 * DISABLE_MESSAGE
 *
 * 특정 메시지를 코어에서 무시하고 라우팅 하지 않도록 비공개화 한다.
 *
 * sMsg string 비공개화 시킬 메시지
 *
---------------------------------------------------------------------------]*/
/*[
 * ENABLE_MESSAGE
 *
 * 무시하도록 설정된 메시지를 무시하지 않도록 공개화 한다.
 *
 * sMsg string 공개화 시킬 메시지
 *
---------------------------------------------------------------------------]*/
/*[
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	 ...
	 * @param {Array} htOptions.aDisabled 비공개화할 버튼명 배열
	 */
	...
	$ON_MSG_APP_READY : function(){
		...
		// [SMARTEDITORSUS-1679] 초기 disabled 처리가 필요한 버튼은 비공개화
		if(this._htOptions.aDisabled){
			this._htOptions._sDisabled = "," + this._htOptions.aDisabled.toString() + ",";	// 버튼을 공개화할때 비교하기 위한 문자열구성 
	...
	/**
	 * [SMARTEDITORSUS-1679] 여러개의 버튼을 동시에 비공개화 할 수 있도록 수정
	 * @param {String|Array} vUIName 비공개화할 버튼명, 배열일 경우 여러개 동시 적용 
	 */
...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	$ON_CLOSE_LAYER_POPUP : function() {
         this.oApp.exec("ENABLE_ALL_UI");                // 모든 UI 공개화.
         this.oApp.exec("DESELECT_UI", ["helpPopup"]);       
         this.oApp.exec("HIDE_ALL_DIALOG_LAYER", []);
         this.oApp.exec("HIDE_EDITING_AREA_COVER");              // 편집 영역 공개화.
 ...
 * REFRESH_WYSIWYG
 *
 * (FF전용) WYSIWYG 모드를 비공개화 후 다시 공개화 시킨다. FF에서 WYSIWYG 모드가 일부 비공개화 되는 문제용
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	$ON_MSG_APP_READY : function(){
		if(this.oApp.bMobile){
			...
			// [SMARTEDITORSUS-1679] 하지만 사용자가 조절하지는 못하도록 버튼은 비공개화 한다.
...
nhn.husky.SE2M_Hyperlink = jindo.$Class({
	...
	/**
	 * [SMARTEDITORSUS-1405] 자동링크 비공개화 옵션을 체크해서 처리한다.
