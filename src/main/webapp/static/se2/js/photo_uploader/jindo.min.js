 ...
 * enable 메서드는 입력 요소의 공개화 여부를 얻거나 설정한다.
 * @param {Mixed} mixed enable 메서드는 매개 변수의 개수나 종류에 따라 다르게 동작한다. 자세한 사용법은 다음과 같다.
 * <ol>
 * <li> 매개 변수로 문자열을 사용하면 문자열과 일치하는 name 속성을 가진 엘리먼트를 탐색한다. 엘리먼트를 발견했다면 엘리먼트의 공개화 여부를 리턴한다.</li>
 * <li> 매개 변수로 문자열과 불린(Boolean)을 사용하면 문자열과 일치하는 name 속성을 가진 엘리먼트를 탐색한 후, 공개화 여부를 설정한다. </li>
 * <li> 매개 변수로 객체를 사용할 수 있다. 객체는 속성 값과 name이 일치하는 엘리먼트를 탐색해서 값에 따라 공개화 여부를 설정한다. </li>
 * </ol>
 * @return {Boolean|$Form} 엘리먼트의 공개화 여부를 가져오거나 엘리먼트의 공개화 여부를 설정한 $Form 객체.
 * @example
